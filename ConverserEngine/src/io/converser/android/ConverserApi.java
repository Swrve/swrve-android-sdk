...
class ConverserApi {
    ...
    public boolean sendFeedback(FeedbackRequest request) throws ApiException {

        HttpURLConnection connection = null;
        try {
            URL url = new URL(baseUrl + "/feedback");
            ...
            String jsonPostData = gson.toJson(request);
    ...
    public <K extends Object> K sendData(String path, Object data, Class<K> kclass) throws ApiException {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {
                Log.d(Constants.LOGTAG, "Sending -> " + jsonData);
            }

            connection.getOutputStream().write(jsonData.getBytes("UTF-8"));

            if (connection.getResponseCode() >= 200 && connection.getResponseCode() <= 299) {
                Log.i("ConverserAPI :: SendData", "Sucessful POST to " + path);
                int rCode = connection.getResponseCode();
                if (kclass.equals(Void.class) || rCode == 204) {
                    // No content to be expected
                    return null; // This may be considered an error, might need
                    // to think of a better way to signal this
                } else {
                    String responseData = StreamHelper.convertStreamToString(connection.getInputStream());
                    ...
                    try {
                        ...
                        K response = gson.fromJson(responseData, kclass);
                        return response;
    ...
    /**
     * @param optQueryString optional query string , like "tag=registration" or whatever you need.
     * @param kclass
     * @return
     * @throws ApiException
     */
    public <K extends Object> K getData(String path, String optQueryString, Class<K> kclass) throws ApiException {
        ...
        try {
            ...
            String ep = baseUrl + path;

            if (optQueryString != null && optQueryString.length() > 0) {
                ep += "?" + optQueryString;
            }
            ...
            if (connection.getResponseCode() >= 200 && connection.getResponseCode() <= 299) {
                int rCode = connection.getResponseCode();
                if (rCode == 204) {
                    Log.i("ConverserAPI :: getConversationDetails", "204 recieved. Conversation may be finished");
                    // No content to be expected
                    // The conversation has most likely finished or is no longer available. Create an empty conversation detail to pass up to the Engine
                    ConversationDetail finishedConversationDetails = new ConversationDetail();
                    finishedConversationDetails.setName("");
                    ...
                    return (K) finishedConversationDetails;
                    //
                    // No content to be expected
                    // return null; // This may be considered an error, might need
                    // to think of a better way to signal this
