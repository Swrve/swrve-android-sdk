...
public class SwrveSQLiteOpenHelperTest extends SwrveBaseTest {
    ...
    public void testNewDatabase() {
        ...
        try (SQLiteDatabase database = sqLiteOpenHelper.getWritableDatabase()) {
            ...
            try (Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='" + EVENTS_TABLE_NAME + "'", null)) {
                assertEquals("Should be 1 table called events in database.", 1, cursor.getCount());
            }
            try (Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='" + CACHE_TABLE_NAME + "'", null)) {
                assertEquals("Should be 1 table called cache in database.", 1, cursor.getCount());
            }
            try (Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='sqlite_sequence'", null)) {
            ...
            try (Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='" + ASSET_LOGS_TABLE_NAME + "'", null)) {
                assertEquals("Should be 1 table called asset_logs in database.", 1, cursor.getCount());
            }
        } catch (Exception ex) {
    ...
    @Test
    public void testNewDatabaseEventsTable() {
        // test brand new instance that table created ok
        String dbName = "testNewDatabaseEventsTable";
        SwrveSQLiteOpenHelper sqLiteOpenHelper = SwrveSQLiteOpenHelper.getInstance(ApplicationProvider.getApplicationContext(), dbName, SWRVE_DB_VERSION);
    ...
    @Test
    public void testNewDatabaseCacheTable() {
        // test brand new instance that table created ok
        String dbName = "testNewDatabaseCacheTable";
        SwrveSQLiteOpenHelper sqLiteOpenHelper = SwrveSQLiteOpenHelper.getInstance(ApplicationProvider.getApplicationContext(), dbName, SWRVE_DB_VERSION);

        try (SQLiteDatabase database = sqLiteOpenHelper.getWritableDatabase();
             Cursor cursor = database.rawQuery("SELECT * FROM " + CACHE_TABLE_NAME, null)) {
            String[] columnNames = cursor.getColumnNames();
    ...
    @Test
    public void testNewDatabaseUserTable() {
        // test brand new instance that table created ok
        String dbName = "testNewDatabaseUserTable";
        SwrveSQLiteOpenHelper sqLiteOpenHelper = SwrveSQLiteOpenHelper.getInstance(ApplicationProvider.getApplicationContext(), dbName, SWRVE_DB_VERSION);
    ...
    @Test
    public void testNewDatabaseOfflineCampaignsTable() {
        // test brand new instance that table created ok
        String dbName = "testNewDatabaseOfflineCampaignsTable";
        SwrveSQLiteOpenHelper sqLiteOpenHelper = SwrveSQLiteOpenHelper.getInstance(ApplicationProvider.getApplicationContext(), dbName, SWRVE_DB_VERSION);

        try (SQLiteDatabase database = sqLiteOpenHelper.getWritableDatabase();
             Cursor cursor = database.rawQuery("SELECT * FROM " + OFFLINE_CAMPAIGNS_TABLE_NAME, null);) {
            String[] columnNames = cursor.getColumnNames();
    ...
    @Test
    public void testNewDatabaseAssetsLogsTable() {
        // test brand new instance that table created ok
        String dbName = "testNewDatabaseAssetsLogsTable";
        SwrveSQLiteOpenHelper sqLiteOpenHelper = SwrveSQLiteOpenHelper.getInstance(ApplicationProvider.getApplicationContext(), dbName, SWRVE_DB_VERSION);

        try (SQLiteDatabase database = sqLiteOpenHelper.getWritableDatabase();
             Cursor cursor = database.rawQuery("SELECT * FROM " + ASSET_LOGS_TABLE_NAME, null);) {
            String[] columnNames = cursor.getColumnNames();
            assertEquals("Should only be 3 columns in asset_logs", 3, columnNames.length);
            assertTrue("onCreate of tables failed as there should be a name column", Arrays.asList(columnNames).contains(ASSET_LOGS_COLUMN_NAME));
            assertTrue("onCreate of tables failed as there should be a download_count column", Arrays.asList(columnNames).contains(ASSET_LOGS_COLUMN_DOWNLOAD_COUNT));
            assertTrue("onCreate of tables failed as there should be a last_download_time column", Arrays.asList(columnNames).contains(ASSET_LOGS_COLUMN_LAST_DOWNLOAD_TIME));
            assertEquals("Should be 0 rows.", 0, cursor.getCount());
        } catch (Exception ex) {
            fail("Exception thrown: " + ex.getMessage());
        } finally {
    ...
    public void testOnUpgradeCache_1_to_2_to_Latest() {
        ...
        try (SwrveSQLiteOpenHelper_v2 swrveSQLiteOpenHelper_v2 = new SwrveSQLiteOpenHelper_v2(ApplicationProvider.getApplicationContext(), dbName);
             SQLiteDatabase database = swrveSQLiteOpenHelper_v2.getWritableDatabase();
             Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='server_cache'", null)) {
        ...
        try (SwrveSQLiteOpenHelper_v2 swrveSQLiteOpenHelper_v2 = new SwrveSQLiteOpenHelper_v2(ApplicationProvider.getApplicationContext(), dbName);
             SQLiteDatabase database = swrveSQLiteOpenHelper_v2.getWritableDatabase();
             Cursor cursor = database.rawQuery("SELECT * FROM sqlite_master WHERE type='table' and name='cache'", null);) {
    ...
    private void assertCacheUserIdUpdated(SQLiteDatabase database, String userId, String categoryValue) {
        try (Cursor cursor = database.rawQuery("SELECT * FROM cache WHERE category='" + categoryValue + "'", null)) {
    ...
    private void assertCacheEntry(int rows, SQLiteDatabase database, String userId, String categoryValue, String rawData) {
        try (Cursor cursor = database.rawQuery("SELECT * FROM cache WHERE category='" + categoryValue + "' AND raw_data='" + rawData + "'", null);) {
    ...
    public void testOnUpgradeEvents_1_to_Latest() {
        ...
        String dbName = "testOnUpgradeEvents_1_to_5";
        ...
        try (SwrveSQLiteOpenHelper_v1 swrveSQLiteOpenHelper_v1 = new SwrveSQLiteOpenHelper_v1(ApplicationProvider.getApplicationContext(), dbName);
            ...
            try (Cursor cursor = database.rawQuery("SELECT * FROM " + EVENTS_TABLE_NAME, null)) {
                String[] columnNames = cursor.getColumnNames();
    ...
    public void testOnUpgradeUsers_1_to_Latest() {
        ...
        String dbName = "testOnUpgradeUsers_1_to_5";
